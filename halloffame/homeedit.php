...
<html lang="en-US">
   ...
   if(isset($_POST['downloadsubmit']))
   {
	   ...
	   {
		    $link3_name=$_FILES['flink3']['name'];
           $link3_type=$_FILES['flink3']['type'];
          $link3path='downloadupload/'.$_FILES['flink3']['name']  ;
		   
		   if($link3_type=='application/pdf' )
		   {
			   $uploadfile=move_uploaded_file($_FILES['flink3']['tmp_name'],'downloadupload/'.$_FILES['flink3']['name']);
			   
			        $sql="UPDATE downloadlink SET `linkname`='$link3_name',`linkpath`='$link3path' where linkno=3  ";
		            $query=mysqli_query($con,$sql);
